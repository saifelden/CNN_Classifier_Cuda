...
__global__ void calc_sum_block(float * input,float* output,Size size){
...
__global__ float calc_sum_all(float* input,float& result){
    int idx = threadIdx.x + (blockIdx.x*blockDim.x);
    float result = 0.0;
    atomicAdd(result,input[idx]);
}

__device__ void calc_mean(float input,Size size,float & mean){
    mean = input/(size.width*size.height*size.channels);
}
__device__ void calc_variance(float* input,Size size,float mean,float& variance){
...
__global__ void calc_batchnorm_forward(float * input,float* output,Size size,float& variance,float& mean,float sum_result,float alfa,float beta){
    int col = threadIdx.x + (blockIdx.x*blockDim.x);
    int row = threadIdx.y + (blockIdx.y*blockDim.y);
    int channels  = threadIdx.z + (blockIdx.z*blockDim.z);

    calc_mean(sum_result,size,mean);
    calc_variance(input,size,mean,variance);
...
__global__ void calc_batchnorm_backward(float* input,Size size, float variance,float mean,float* deriv_0,float* deriv_1){
    int col = threadIdx.x + (blockIdx.x*blockDim.x);
    int row = threadIdx.y + (blockIdx.y*blockDim.y);
    int channels  = threadIdx.z + (blockIdx.z*blockDim.z);
    ...
    float first_phrase;
    calc_first_phrase(input,size,mean,first_phrase);
